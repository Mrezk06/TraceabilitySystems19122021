...
Public Class Heater_Registration__step_four
    ...
    Private Function _Refresh1() As Boolean
        Try
            Dim sql As String = ""
            sql += " SELECT Model_Name, count(Model_Name) AS tot"
            sql += " FROM TH_v_new "
            sql += " WHERE H_date >= Convert(nvarchar(10), GETDATE(), 121)"
            sql += " and H_Line_Name='H1'"
            '    sql += " and fLCDPline ='" & txtline.Text & "'"
            sql += " GROUP BY Model_Name "
            Dim da As New SqlClient.SqlDataAdapter(sql, cn)
            Dim ds As New DataSet
            da.Fill(ds, "TH_v_new")
            ds.Tables("TH_v_new").Columns(0).ColumnName = " الموديل "
            ds.Tables("TH_v_new").Columns(1).ColumnName = " الكمية"
            dg2.DataSource = ds.Tables("TH_v_new")
        ...
        Try
            If e.KeyCode = Keys.Enter Then
                ...
                Else
                    ...
                    sql += "VALUES     ('" & cmb_Pcode.Text & "','" & txtBarcodeOne.Text & "','" & txtPARTSSERIAL.Text & "','" & txtshift.Text & "','" & txtParts1.Text & "','H1')"
        ...
        Try
            If e.KeyCode = Keys.Enter Then
                ...
                Else
                    ...
                    sql = "INSERT INTO Heater_registration_Three"
                    sql += "( H_Model_Name,H_Heater_Code,H_component,H_Shift_Line,H_Name_Laber,H_Line_Name)"
                    sql += "VALUES     ('" & lbl_Pcode_Value.Text & "','" & txtBarcodeOne.Text & "','" & txtPARTSSERIAL.Text & "','" & txtshift.Text & "','" & txtParts1.Text & "','H1')"
