...
Public Class frmSHA_InputQuery
    ...
    Private Function _Refresh001() As Boolean
        Try
            ...
            'Dim sql As String = ""
            sql += " SELECT count(CBarcode) FROM  SHA_Input_Production "
            ...
            ds.Tables("SHA_Input_Production").Columns(0).ColumnName = " الكمية "
            dg6.DataSource = ds.Tables("SHA_Input_Production")
    ...
    Private Function _Refresh111() As Boolean
        Try
            ...
            Dim sql As String = ""
            sql += " SELECT CDate,Ctime,CBarcode,CEAN_Code,CModelName,C_Line_Shift,CSapUser FROM  SHA_Input_Production "
            sql += "   where CDate>= '" & Format(dtpFROMDATE.Value, "yyyy-MM-dd") & "'"
            sql += "   and CDate<= '" & Format(dtpToDATE.Value, "yyyy-MM-dd") & "'"
            sql += "   and Ctime>='" & txttotime.Text & "'"
            sql += "   and Ctime<= '" & txtfromTime.Text & "'"
            ...
            Dim ds As New DataSet
            da.Fill(ds, "SHA_Input_Production")
            ds.Tables("SHA_Input_Production").Columns(0).ColumnName = " التاريخ "
            ds.Tables("SHA_Input_Production").Columns(1).ColumnName = " الوقت "
            ds.Tables("SHA_Input_Production").Columns(2).ColumnName = " باركود الجهاز"
            ds.Tables("SHA_Input_Production").Columns(3).ColumnName = " الايان كود"
            ds.Tables("SHA_Input_Production").Columns(4).ColumnName = " الموديل"
            ds.Tables("SHA_Input_Production").Columns(5).ColumnName = "الخط والورديه"
            ds.Tables("SHA_Input_Production").Columns(6).ColumnName = " رقم ساب المسئول "
            dg.DataSource = ds.Tables("SHA_Input_Production")
    ...
    Private Function _Refresh155() As Boolean
        Try
            ...
            Dim ds As New DataSet
            da.Fill(ds, "SHA_Input_Production")
            ds.Tables("SHA_Input_Production").Columns(0).ColumnName = "الخط"
            ds.Tables("SHA_Input_Production").Columns(1).ColumnName = "الموديل"
            ds.Tables("SHA_Input_Production").Columns(2).ColumnName = " الكمية"
            dg9.DataSource = ds.Tables("SHA_Input_Production")
