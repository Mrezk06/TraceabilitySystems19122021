...
Public Class frmSHA_AddModel
    Private Sub Button2_Click(sender As Object, e As EventArgs)
        'Close()
    End Sub
    Private Function Add1_New() As Boolean
        Try
            Dim dd As String = TxtSet.Text + txtArea.Text
            Dim sql As String
            sql = "	INSERT INTO SHA_Models "
            sql += "(CSetID, CModelName, CSetIDAndLine, CEanCode, CUserID, Cline,ProductName)"
            sql += " VALUES ('" & TxtSet.Text & "','" & txtModel.Text & "','" & dd & "','" & txtsteker_power.Text & "','" & Label10.Text & "','" & txtArea.Text & "','" & txtProductName.Text & "')"
    ...
    Private Function _Refresh1() As Boolean
        Try
            Dim sql1 As String = "SELECT  CSetID, CModelName, CSetIDAndLine, CEanCode, CUserID, Cline,ProductName FROM SHA_Models"
            Dim da As New SqlClient.SqlDataAdapter(sql1, cn)
            Dim ds As New DataSet
            da.Fill(ds, "SHA_Models")
            ds.Tables("SHA_Models").Columns(0).ColumnName = "Model_Control"
            ds.Tables("SHA_Models").Columns(1).ColumnName = "Model_Name"
            ds.Tables("SHA_Models").Columns(2).ColumnName = "Model_Control + Line"
            ds.Tables("SHA_Models").Columns(3).ColumnName = "Ean Code "
            ds.Tables("SHA_Models").Columns(4).ColumnName = "Create BY "
            ds.Tables("SHA_Models").Columns(5).ColumnName = "Line"
            ds.Tables("SHA_Models").Columns(6).ColumnName = "ProductName"
            dg.DataSource = ds.Tables("SHA_Models")
    ...
    Private Sub Heater_add_new_model_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            Dim sql As String = ""
            sql += "SELECT ProductName FROM SHA_Products"
            '  sql += " where fLine_Name ='EWH1'"
            Dim da As New SqlClient.SqlDataAdapter(sql, cn)
            Dim ds As New DataSet
            ds.Tables.Clear()
            da.Fill(ds, "SHA_Products")
            txtProductName.DataSource = ds.Tables(0)
            ' cmb_Pcode.ValueMember = "CSetID"
            '  txtProductName.ValueMember = "CSetID"
            txtProductName.DisplayMember = "ProductName"
            txtProductName.Sorted = True
            '_Refresh1()
        Catch ex As Exception
        End Try
        ' Me.SkinEngine1.SkinFile = "Skins/EmeraldColor1.ssk"
        Label10.Text = StringPasscx
    End Sub
    ...
    Private Sub GroupBox2_Enter(sender As Object, e As EventArgs) Handles GroupBox2.Enter
        ...
        _Refresh315()
    End Sub
    Public Property StringPasscx As String
    ...
    Private Sub dg_Click(sender As Object, e As EventArgs) Handles dg.Click
        ...
        TxtSet.Text = dg.Item(2, dg.CurrentRow.Index).Value
        txtsteker_power.Text = dg.Item(3, dg.CurrentRow.Index).Value
        txtArea.Text = dg.Item(5, dg.CurrentRow.Index).Value
    End Sub
