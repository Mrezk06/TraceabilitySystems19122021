...
Public Class frmSHA_RepairQuery
    ...
    Private Function _Refreshdgd() As Boolean
        Try
            Dim sql As String = ""
            sql += "SELECT R_date,R_Time,  R_Barcode1, R_ModelName, R_Name_Problem, Expr1, R_Type_Problem, R_Line, R_Name_Laber, R_SV FROM  SHA_Repair"
            sql += " where R_Barcode1='" & txtbarcode.Text & "'"
            Dim da As New SqlClient.SqlDataAdapter(sql, cn)
            Dim ds As New DataSet
            da.Fill(ds, "SHA_Repair")
            ' ds.Tables("R_MainRepair").Columns(0).ColumnName = "ModelName"
            ds.Tables("SHA_Repair").Columns(0).ColumnName = "التاريخ"
            ds.Tables("SHA_Repair").Columns(1).ColumnName = "الوقت"
            ds.Tables("SHA_Repair").Columns(2).ColumnName = "الباركود"
            ds.Tables("SHA_Repair").Columns(3).ColumnName = "الموديل"
            ds.Tables("SHA_Repair").Columns(4).ColumnName = "اسم المشكلة"
            ds.Tables("SHA_Repair").Columns(5).ColumnName = "سبب المشكلة"
            ds.Tables("SHA_Repair").Columns(6).ColumnName = "نوع المشكلة"
            ds.Tables("SHA_Repair").Columns(7).ColumnName = "الخط والوردية"
            ds.Tables("SHA_Repair").Columns(8).ColumnName = "اسم سبب المشكلة"
            ds.Tables("SHA_Repair").Columns(9).ColumnName = "اسم المشرف"
            dg9.DataSource = ds.Tables("SHA_Repair")
    ...
    Private Function _RefreshdgdDatem() As Boolean
        Try
            Dim sql As String = ""
            sql += "SELECT R_ModelName, count(R_ModelName) FROM  SHA_Repair"
            sql += "   where R_date>= '" & Format(dtpFROMDATE.Value, "yyyy-MM-dd") & "'"
            sql += "   and R_date<= '" & Format(DateTimePicker1.Value, "yyyy-MM-dd") & "'"
            ...
            Dim ds As New DataSet
            da.Fill(ds, "SHA_Repair")
            ds.Tables("SHA_Repair").Columns(0).ColumnName = "الموديل"
            ds.Tables("SHA_Repair").Columns(1).ColumnName = "الكمية"
            dgf.DataSource = ds.Tables("SHA_Repair")
    ...
    Private Function _RefreshdgdDateq() As Boolean
        Try
            Dim sql As String = ""
            sql += "SELECT count( R_Barcode1) FROM  SHA_Repair"
            sql += "   where R_date>= '" & Format(dtpFROMDATE.Value, "yyyy-MM-dd") & "'"
            sql += "   and R_date<= '" & Format(DateTimePicker1.Value, "yyyy-MM-dd") & "'"
            Dim da As New SqlClient.SqlDataAdapter(sql, cn)
            Dim ds As New DataSet
            da.Fill(ds, "SHA_Repair")
            ds.Tables("SHA_Repair").Columns(0).ColumnName = "الكمية"
            dgq.DataSource = ds.Tables("SHA_Repair")
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        ...
        connection.ConnectionString = "Data Source=10.12.112.233;Initial Catalog=barcode;User ID=sa;Password=a-123456"
        command.Connection = connection
        command.CommandType = CommandType.Text
        'You can use any command select
        command.CommandText = "SELECT R_ModelName, R_SV, R_Barcode1, R_Line, R_Type_Problem, R_SN_ProblemLaber, R_Time, R_date, R_Name_Problem, Expr1 FROM SHA_Repair where R_ModelName ='" & txtmodel.Text & "'"
