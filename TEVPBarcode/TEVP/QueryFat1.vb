...
Public Class QueryFat1
    ...
    Private Function _Refresh001() As Boolean
        Try
            ...
            sql += " SELECT count( fLCDSN) FROM DailyP "
            ...
            da.Fill(ds, "DailyP")
            ds.Tables("DailyP").Columns(0).ColumnName = " summation "
    ...
    Private Function _Refresh111() As Boolean
        Try
            ...
            Dim sql As String = ""
            sql += " SELECT  fPDate, fPtime, fLCDBarcode, fLCDModelName, fLCDPline, fLCDSN, fQty FROM  DailyP "
            sql += "   where fPDate>= '" & Format(dtpFROMDATE.Value, "yyyy-MM-dd") & "'"
            sql += "   and fPDate<= '" & Format(DateTimePicker1.Value, "yyyy-MM-dd") & "'"
            'sql += "   and fPtime>='" & txttotime.Text & "'"
            'sql += "   and fPtime<= '" & txtfromTime.Text & "'"
    ...
    Private Function _Refresh1() As Boolean
        Try
            Dim sql As String = ""
            sql += " SELECT  fPDate, fPtime, fLCDBarcode, fLCDModelName, fLCDPline, fLCDSN, fQty,fShift FROM  DailyP "
            sql += " where fLCDBarcode ='" & txtFATSERIAL.Text & "'"
            Dim da As New SqlClient.SqlDataAdapter(sql, cn)
            Dim ds As New DataSet
            da.Fill(ds, "DailyP")
    ...
    Private Function _Refresh155() As Boolean
        Try
            ...
            dg9.DataSource = ds.Tables("DailyP")
            Return True
        Catch ex As Exception
        End Try
    End Function
