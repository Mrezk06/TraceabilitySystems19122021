...
Public Class frmQuery_PCB_Input
    ...
    Private Function _Refresh001() As Boolean
        Try
            ...
            sql += " SELECT count(fLCDSetID) FROM PCBIN "
            ...
            da.Fill(ds, "PCBIN")
            ds.Tables("PCBIN").Columns(0).ColumnName = " summation "
            ...
            dg6.DataSource = ds.Tables("PCBIN")
    ...
    Private Function _Refresh511() As Boolean
        Try
            ...
            sql += " SELECT count( fLCDBarcode) FROM  PCBIN "
    ...
    Private Function _Refresh111() As Boolean
        Try
            ...
            Dim sql As String = ""
            sql += " SELECT fPDate, fPtime, fLCDBarcode, fLCDModelName,fLine_and_shift, fSap FROM  PCBIN "
            sql += "   where fPDate>= '" & Format(dtpFROMDATE.Value, "yyyy-MM-dd") & "'"
            sql += "   and fPDate<= '" & Format(DateTimePicker1.Value, "yyyy-MM-dd") & "'"
            'sql += "   and fPtime>='" & txttotime.Text & "'"
            'sql += "   and fPtime<= '" & txtfromTime.Text & "'"
            ...
            Dim ds As New DataSet
            da.Fill(ds, "PCBIN")
          ds.Tables("PCBIN").Columns(0).ColumnName = " Date "
            ds.Tables("PCBIN").Columns(1).ColumnName = " time "
            ds.Tables("PCBIN").Columns(2).ColumnName = "  Barcode"
            ds.Tables("PCBIN").Columns(3).ColumnName = " Model Name"
            ds.Tables("PCBIN").Columns(4).ColumnName = "line"
            ds.Tables("PCBIN").Columns(5).ColumnName = "Sap"
            dg.DataSource = ds.Tables("PCBIN")
            Return True
        Catch ex As Exception
        End Try
    ...
    Private Function _Refresh1() As Boolean
        Try
            Dim sql As String = ""
            sql += " SELECT  SELECT  fPDate, fPtime, fLCDBarcode, fLCDModelName,fLine_and_shift, fSap FROM PCBIN "
            sql += " where fLCDBarcode ='" & txtFATSERIAL.Text & "'"
            Dim da As New SqlClient.SqlDataAdapter(sql, cn)
            Dim ds As New DataSet
            da.Fill(ds, "PCBIN")
    ...
    Private Function _Refresh155() As Boolean
        Try
            ...
            Dim ds As New DataSet
            da.Fill(ds, "PCBIN")
            ds.Tables("PCBIN").Columns(0).ColumnName = " LCD Model Name"
            ds.Tables("PCBIN").Columns(1).ColumnName = " Qty"
            dg9.DataSource = ds.Tables("PCBIN")
            Return True
        Catch ex As Exception
        End Try
    End Function
    ...
    Private Function _Refresh199() As Boolean
        Try
            ...
            ds.Tables("PCBIN").Columns(1).ColumnName = " Qty"
            ds.Tables("PCBIN").Columns(2).ColumnName = "line"
            dg9.DataSource = ds.Tables("PCBIN")
            Return True
        Catch ex As Exception
        End Try
