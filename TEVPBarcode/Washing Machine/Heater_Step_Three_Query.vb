...
Public Class Heater_Step_Three_Query
    ...
    Private Function _Refresh001() As Boolean
        Try
            ...
            sql += " SELECT count(H_Heater_Code) FROM  Heater_registration_Two "
    ...
    Private Function _Refresh111() As Boolean
        Try
            ...
            Dim sql As String = ""
            sql += " SELECT H_date, H_time, Model_Name, H_Heater_Code, H_Shift_Line,Heater_Name_Laber, H_Name_Laber FROM  TW_V_new "
            sql += "   where H_date>= '" & Format(dtpFROMDATE.Value, "yyyy-MM-dd") & "'"
            sql += "   and H_date<= '" & Format(dtpToDATE.Value, "yyyy-MM-dd") & "'"
            sql += "   and H_time>='" & txttotime.Text & "'"
            sql += "   and H_time<= '" & txtfromTime.Text & "'"
            ...
            Dim ds As New DataSet
            da.Fill(ds, "TW_V_new")
            ds.Tables("TW_V_new").Columns(0).ColumnName = " التاريخ "
            ds.Tables("TW_V_new").Columns(1).ColumnName = " الوقت "
            ds.Tables("TW_V_new").Columns(2).ColumnName = " الموديل"
            ds.Tables("TW_V_new").Columns(3).ColumnName = " باركود الجهاز"
            ds.Tables("TW_V_new").Columns(4).ColumnName = "الخط والورديه"
            ds.Tables("TW_V_new").Columns(5).ColumnName = "اسم المسئول"
            ds.Tables("TW_V_new").Columns(6).ColumnName = " رقم ساب المسئول "
            '  ds.Tables("Heater_registration_Two").Columns(6).ColumnName = "Qty"
            dg.DataSource = ds.Tables("TW_V_new")
    ...
    Private Function _Refresh11() As Boolean
        Try
            ...
            'Dim sql As String = ""
            'sql += " SELECT  H_date, H_time, H_Model_Name, H_Heater_Code, H_Name_Laber, H_Shift_Line FROM Heater_registration_Two "
            ...
            'Dim ds As New DataSet
            'da.Fill(ds, "Heater_registration_Two")
            'ds.Tables("Heater_registration_Two").Columns(0).ColumnName = " التاريخ "
            'ds.Tables("Heater_registration_Two").Columns(1).ColumnName = " الوقت "
            'ds.Tables("Heater_registration_Two").Columns(2).ColumnName = " الموديل"
            'ds.Tables("Heater_registration_Two").Columns(3).ColumnName = " باركود الجهاز"
            'ds.Tables("Heater_registration_Two").Columns(4).ColumnName = "رقم ساب المسئول"
            'ds.Tables("Heater_registration_Two").Columns(5).ColumnName = " الخط والوردية"
            ''  ds.Tables("Heater_registration_Two").Columns(6).ColumnName = "Qty"
            'dg.DataSource = ds.Tables("Heater_registration_Two")
            ...
            'sql += "   and H_time>='" & txttotime.Text & "'"
            'sql += "   and H_time<= '" & txtfromTime.Text & "'"
            'sql += "   and Model_Name ='" & btnPCode.Text & "'"
            'sql += "   and fPtime<= '" & Format(txttotime.ToString, "hh:mm:ss") & "'"
            Dim da As New SqlClient.SqlDataAdapter(sql, cn)
            Dim ds As New DataSet
            da.Fill(ds, "TW_V_new")
    ...
    Private Function _Refresh1() As Boolean
        Try
            ...
            'Dim sql As String = ""
            'sql += " SELECT  H_date, H_time, H_Model_Name, H_Heater_Code, H_Shift_Line,H_Name_Laber FROM Heater_registration_Two "
            'sql += " where H_Heater_Code ='" & txtFATSERIAL.Text & "'"
            ''sql += "   and H_date= '" & Format(dtpFROMDATE.Value, "yyyy-MM-dd") & "'"
            ''sql += "   and H_time>='" & txttotime.Text & "'"
            ''sql += "   and H_time<= '" & txtfromTime.Text & "'"
            ...
            'ds.Tables("Heater_registration_Two").Columns(2).ColumnName = " الموديل"
            'ds.Tables("Heater_registration_Two").Columns(3).ColumnName = " باركود الجهاز"
            ''  ds.Tables("Heater_registration_Two").Columns(4).ColumnName = "H_component"
            'ds.Tables("Heater_registration_Two").Columns(4).ColumnName = " الخط والوردية"
            'ds.Tables("Heater_registration_Two").Columns(5).ColumnName = "رقم ساب المسئول"
            'dg.DataSource = ds.Tables("Heater_registration_Two")
    ...
    Private Function _Refresh155() As Boolean
        Try
            ...
            Dim ds As New DataSet
            da.Fill(ds, "MVData")
            ds.Tables("MVData").Columns(0).ColumnName = "الخط"
            ds.Tables("MVData").Columns(1).ColumnName = "الموديل"
            ds.Tables("MVData").Columns(2).ColumnName = " الكمية"
            dg9.DataSource = ds.Tables("MVData")
            Return True
        Catch ex As Exception
        End Try
    End Function
