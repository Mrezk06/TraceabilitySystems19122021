...
Public Class Heater_Step_Two_Query
    ...
    Private Function _Refresh001() As Boolean
        Try
            ...
            sql += " SELECT count(H_Heater_Code) FROM  barcode.dbo.Heater_registration_Three "
    ...
    Private Function _Refresh111() As Boolean
        Try
            ...
            Dim sql As String = ""
            sql += "SELECT  H_date, H_time, Model_Name, H_Heater_Code, H_component, H_Shift_Line, Heater_Name_Laber, H_Name_Laber FROM TH_v_new "
            sql += "   where H_date>= '" & Format(dtpFROMDATE.Value, "yyyy-MM-dd") & "'"
            sql += "   and H_date<= '" & Format(dtpToDATE.Value, "yyyy-MM-dd") & "'"
            'sql += "   and H_date= '" & Format(dtpFROMDATE.Value, "yyyy-MM-dd") & "'"
            'sql += "   and H_time>='" & txttotime.Text & "'"
            ...
            Dim ds As New DataSet
            da.Fill(ds, "TH_v_new")
            ds.Tables("TH_v_new").Columns(0).ColumnName = " التاريخ "
            ds.Tables("TH_v_new").Columns(1).ColumnName = " الوقت "
            ds.Tables("TH_v_new").Columns(2).ColumnName = " الموديل"
            ds.Tables("TH_v_new").Columns(3).ColumnName = " باركود الجهاز"
            ds.Tables("TH_v_new").Columns(4).ColumnName = "الايان كود"
            ds.Tables("TH_v_new").Columns(5).ColumnName = " الخط والوردية"
            ds.Tables("TH_v_new").Columns(6).ColumnName = "أسم المسئول"
            ds.Tables("TH_v_new").Columns(7).ColumnName = "رقم ساب المسئول"

            dg.DataSource = ds.Tables("TH_v_new")
            Return True
        Catch ex As Exception
        End Try
    ...
    Private Function _Refresh155() As Boolean
        Try
            ...
            Dim ds As New DataSet
            da.Fill(ds, "MVData2")
            ds.Tables("MVData2").Columns(0).ColumnName = " الخط"
            ds.Tables("MVData2").Columns(1).ColumnName = "الموديل"
            ds.Tables("MVData2").Columns(2).ColumnName = " الكميه"
            dg9.DataSource = ds.Tables("MVData2")
            Return True
        Catch ex As Exception
        End Try
    End Function
