...
Public Class frmQFinishGoodWH1
    ...
    Private Function _Refresh1() As Boolean
        Try
            ...
            DG1.DataSource = ds.Tables("OutputWH1")
            Return True
    ...
    Private Function _Refresh1dd() As Boolean
        Try
            ...
            Dim ds As New DataSet
            da.Fill(ds, "OutputWH1")
            ds.Tables("OutputWH1").Columns(0).ColumnName = " LCD Model Name"
            ds.Tables("OutputWH1").Columns(1).ColumnName = " Qty"
            DG1.DataSource = ds.Tables("OutputWH1")
            Return True
        Catch ex As Exception
    ...
    Private Function _Refresh13() As Boolean
        Try
            ...
            dg3.DataSource = ds.Tables("OutputWH1")
            Return True
        Catch ex As Exception
        End Try
    End Function
    ...
    Private Function _Refresh25() As Boolean
        Try
            ...
            da.Fill(ds, "OutputWH1")
            ds.Tables("OutputWH1").Columns(0).ColumnName = " LCD Model Name"
            ds.Tables("OutputWH1").Columns(1).ColumnName = " LCDSetID"
            ds.Tables("OutputWH1").Columns(2).ColumnName = "LCD Barcode"
            ds.Tables("OutputWH1").Columns(3).ColumnName = " Date"
            ds.Tables("OutputWH1").Columns(4).ColumnName = " LCDPline"
            ds.Tables("OutputWH1").Columns(5).ColumnName = "NameL"
    ...
    Private Function _Refreshn3() As Boolean
        Try
            Dim sql As String = ""
            sql += "SELECT  fPDate,fPtime, fModel, fLCDBarcode, fLCDPyear, fLCDPweek, fLCDCountry, fLCDPline, fLCDPID, fLCDSN, fNameL FROM OutputWH1"
            sql += "   WHERE fPDate>= '" & Format(dtbfrom.Value, "yyyy-MM-dd") & "'"
            sql += "   and fPDate<= '" & Format(dtbto.Value, "yyyy-MM-dd") & "'"
            ' sql += " and fLCDModelName='" & cmb_Pcode.Text & "'"
            '    sql += " and fLCDPline='" & ComboBox1.Text & "'"
            ...
            Dim ds As New DataSet
            da.Fill(ds, "OutputWH1")
            ds.Tables("OutputWH1").Columns(0).ColumnName = "Data "
            ds.Tables("OutputWH1").Columns(1).ColumnName = "Time"
            ds.Tables("OutputWH1").Columns(2).ColumnName = " Model Name"
            ds.Tables("OutputWH1").Columns(3).ColumnName = " Barcode"
            ds.Tables("OutputWH1").Columns(4).ColumnName = "LCD P year"
            ds.Tables("OutputWH1").Columns(5).ColumnName = "LCD p Week"
            ' ds.Tables("View_4").Columns(5).ColumnName = "LCD Set ID "
            ds.Tables("OutputWH1").Columns(6).ColumnName = "LCD Country"
            ds.Tables("OutputWH1").Columns(7).ColumnName = "LCD P line"
            ds.Tables("OutputWH1").Columns(8).ColumnName = " fLCDPID"
            ds.Tables("OutputWH1").Columns(9).ColumnName = " SN"
            ds.Tables("OutputWH1").Columns(10).ColumnName = "Name shift"
            dgs.DataSource = ds.Tables("OutputWH1")
            Return True
        Catch ex As Exception
        End Try
    End Function
    ...
    Private Function _Refresh45() As Boolean
        Try
            Dim sql As String = ""
            sql += " SELECT  fModel,COUNT (fModel)  FROM  OutputWH1"
            sql += "   WHERE fPDate>= '" & Format(dtbfrom.Value, "yyyy-MM-dd") & "'"
            sql += "   and fPDate<= '" & Format(dtbto.Value, "yyyy-MM-dd") & "'"
            sql += " and fModel='" & cmb_Pcode.Text & "'"
            sql += " and fLCDPline='" & ComboBox1.Text & "'"
    ...
    Private Function _Refresh44() As Boolean
        Try
            Dim sql As String = ""
            sql += " SELECT  COUNT (fModel)  FROM  OutputWH1"
            sql += "   WHERE fPDate>= '" & Format(dtbfrom.Value, "yyyy-MM-dd") & "'"
            sql += "   and fPDate<= '" & Format(dtbto.Value, "yyyy-MM-dd") & "'"
            sql += " and fModel='" & cmb_Pcode.Text & "'"
            sql += " and fLCDPline='" & ComboBox1.Text & "'"
