...
Public Class frmHeaterRepairQuery
    ...
    Private Function _Refreshdgd() As Boolean
        Try
            Dim sql As String = ""
            sql += "SELECT R_date, R_time, R_BarcodeP1, R_BarcodeP2, R_ModelName, H_Shift_Line, R_Name, Expr1, Heater_Name_Laber, R_Name_Laber, H_Line_Name, R_Note FROM  Heater_Repair_data"
            sql += " where R_BarcodeP1='" & txtbarcode.Text & "'"
            Dim da As New SqlClient.SqlDataAdapter(sql, cn)
            Dim ds As New DataSet
            da.Fill(ds, "Heater_Repair_data")
            ds.Tables("Heater_Repair_data").Columns(0).ColumnName = "التاريخ"
            ds.Tables("Heater_Repair_data").Columns(1).ColumnName = "الوقت"
            ds.Tables("Heater_Repair_data").Columns(2).ColumnName = "الباركود"
            ds.Tables("Heater_Repair_data").Columns(3).ColumnName = "الباركود الجديد"
            ds.Tables("Heater_Repair_data").Columns(4).ColumnName = "الموديل"
            ds.Tables("Heater_Repair_data").Columns(5).ColumnName = "الخط والوردية"
            ds.Tables("Heater_Repair_data").Columns(6).ColumnName = "اسم المشكلة"
            ds.Tables("Heater_Repair_data").Columns(7).ColumnName = "سبب المشكلة"
            ds.Tables("Heater_Repair_data").Columns(8).ColumnName = "مسئول ادخال البيانات"
            ds.Tables("Heater_Repair_data").Columns(9).ColumnName = "اسم المتسبب فى المشكلة"
            ds.Tables("Heater_Repair_data").Columns(10).ColumnName = "نوع المشكلة"
            ds.Tables("Heater_Repair_data").Columns(11).ColumnName = "الملاحظات"
            dg9.DataSource = ds.Tables("Heater_Repair_data")
            Return True
        Catch ex As Exception
        End Try
    End Function
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        ...
        If txtline.Text = "" And txtmodel.Text = "" And txtstetue.Text = "" Then
            _RefreshdgdDate()
            _RefreshdgdDateq()
            _RefreshdgdDatem()

        ElseIf txtline.Text = "" And txtstetue.Text = "" Then

            _RefreshdgdDate1()
            _RefreshdgdDate1q()
            _RefreshdgdDate1M()
        ElseIf txtmodel.Text = "" And txtstetue.Text = "" Then

            _RefreshdgdDate1x()
            _RefreshdgdDate1qx()
            _RefreshdgdDate1Mx()
        ElseIf txtstetue.Text = "" Then
            _RefreshdgdDate2()
            _RefreshdgdDate2q()
            _RefreshdgdDate2qm()
        Else
    ...
    Private Function _RefreshdgdDatem() As Boolean
        Try
            Dim sql As String = ""
            sql += "SELECT R_ModelName, count(R_ModelName) FROM  Heater_Repair_data"
            sql += "   where R_date>= '" & Format(dtpFROMDATE.Value, "yyyy-MM-dd") & "'"
            sql += "   and R_date<= '" & Format(DateTimePicker1.Value, "yyyy-MM-dd") & "'"
            ...
            Dim ds As New DataSet
            da.Fill(ds, "Heater_Repair_data")
            ds.Tables("Heater_Repair_data").Columns(0).ColumnName = "الموديل"
            ds.Tables("Heater_Repair_data").Columns(1).ColumnName = "الكمية"
            dgf.DataSource = ds.Tables("Heater_Repair_data")
            Return True
        Catch ex As Exception
        End Try
    End Function
    Private Function _RefreshdgdDateq() As Boolean
        Try
            Dim sql As String = ""
            sql += "SELECT count( R_BarcodeP1) FROM  Heater_Repair_data"
            sql += "   where R_date>= '" & Format(dtpFROMDATE.Value, "yyyy-MM-dd") & "'"
            sql += "   and R_date<= '" & Format(DateTimePicker1.Value, "yyyy-MM-dd") & "'"
            Dim da As New SqlClient.SqlDataAdapter(sql, cn)
            Dim ds As New DataSet
            da.Fill(ds, "Heater_Repair_data")
            ds.Tables("Heater_Repair_data").Columns(0).ColumnName = "الكمية"
            dgq.DataSource = ds.Tables("Heater_Repair_data")
            Return True
        Catch ex As Exception
        End Try
    End Function
    ...
    Private Function _RefreshdgdDate2() As Boolean
        Try
            ...
            sql += "   and R_date<= '" & Format(DateTimePicker1.Value, "yyyy-MM-dd") & "'"
            sql += " and R_ModelName='" & txtmodel.Text & "'"
            sql += " and H_Shift_Line='" & txtline.Text & "'"
            'sql += "   where R_date>= '" & Format(dtpFROMDATE.Value, "yyyy-MM-dd") & "'"
            'sql += "   and R_date<= '" & Format(DateTimePicker1.Value, "yyyy-MM-dd") & "'"
            'sql += " and R_ModelName='" & txtmodel.Text & "'"
            'sql += " and R_Line='" & txtline.Text & "'"
    ...
    Private Function _RefreshdgdDate2q() As Boolean
        Try
            Dim sql As String = ""
            sql += "SELECT count(R_ModelName) FROM  Heater_Repair_data"
      sql += "   where R_date>= '" & Format(dtpFROMDATE.Value, "yyyy-MM-dd") & "'"
            sql += "   and R_date<= '" & Format(DateTimePicker1.Value, "yyyy-MM-dd") & "'"
            sql += " and R_ModelName='" & txtmodel.Text & "'"
            sql += " and H_Shift_Line='" & txtline.Text & "'"
    ...
    Private Function _RefreshdgdDate3() As Boolean
        Try
            ...
            sql += " and R_ModelName='" & txtmodel.Text & "'"
            sql += " and H_Shift_Line='" & txtline.Text & "'"
            sql += " and H_Line_Name='" & txtstetue.Text & "'"
            Dim da As New SqlClient.SqlDataAdapter(sql, cn)
            Dim ds As New DataSet
            da.Fill(ds, "Heater_Repair_data")
            ds.Tables("Heater_Repair_data").Columns(0).ColumnName = "التاريخ"
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        ...
        command.CommandText = "SELECT R_date, R_time, R_BarcodeP1, R_BarcodeP2, R_ModelName, H_Shift_Line, R_Name, Expr1, Heater_Name_Laber, R_Name_Laber, H_Line_Name, R_Note FROM Heater_Repair_data where R_ModelName ='" & txtmodel.Text & "'"
