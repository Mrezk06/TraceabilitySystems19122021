...
Public Class FrmHeaterStepThreeQuery
    ...
    Private Function _Refresh001() As Boolean
        Try
            ...
            sql += " SELECT count(H_Heater_Code) FROM  Heater_registration_Three1 "
            ...
            ds.Tables("Heater_registration_Three1").Columns(0).ColumnName = " summation "
            dg6.DataSource = ds.Tables("Heater_registration_Three1")
            Return True
    ...
    Private Function _Refresh111() As Boolean
        Try
            ...
            Dim sql As String = ""
            sql += " SELECT  H_date, H_time, H_Model_Name, H_Heater_Code, H_PCB_C, H_Shift_Line, H_Name_Laber, H_EANCode FROM  Heater_registration_Three1 "
            sql += "   where H_date= '" & Format(dtpFROMDATE.Value, "yyyy-MM-dd") & "'"
            sql += "   and H_time>='" & txttotime.Text & "'"
            sql += "   and H_time<= '" & txtfromTime.Text & "'"
            Dim da As New SqlClient.SqlDataAdapter(sql, cn)
            Dim ds As New DataSet
            da.Fill(ds, "Heater_registration_Three1")
            ds.Tables("Heater_registration_Three1").Columns(0).ColumnName = "Date "
            ds.Tables("Heater_registration_Three1").Columns(1).ColumnName = "Time "
            ds.Tables("Heater_registration_Three1").Columns(2).ColumnName = "Model Name"
            ds.Tables("Heater_registration_Three1").Columns(3).ColumnName = "H_Heater_Code"
            ds.Tables("Heater_registration_Three1").Columns(4).ColumnName = "H_PCB_C"
            ' ds.Tables("Heater_registration_Three1").Columns(4).ColumnName = "H_Tank"
            ds.Tables("Heater_registration_Three1").Columns(5).ColumnName = "Shift_Line"
            ds.Tables("Heater_registration_Three1").Columns(6).ColumnName = "sap"
            ds.Tables("Heater_registration_Three1").Columns(7).ColumnName = "Ean Code"
            dg.DataSource = ds.Tables("Heater_registration_Three1")
            Return True
        Catch ex As Exception
        End Try
    End Function
    Private Function _Refresh11() As Boolean
        Try
            ...
            'Dim sql As String = ""
            'sql += " SELECT  H_date, H_time, H_Model_Name, H_Heater_Code, H_Tank, H_Shift_Line, H_sap FROM  barcode.dbo.Heater_registration_Three1 "
            'sql += " where H_Model_Name ='" & btnPCode.Text & "'"
            'sql += "   and H_date= '" & Format(dtpFROMDATE.Value, "yyyy-MM-dd") & "'"
            'sql += "   and H_time>='" & txttotime.Text & "'"
            'sql += "   and H_time<= '" & txtfromTime.Text & "'"
            ...
            'ds.Tables("Heater_registration_Three1").Columns(1).ColumnName = "time "
            'ds.Tables("Heater_registration_Three1").Columns(2).ColumnName = "Model Name"
            'ds.Tables("Heater_registration_Three1").Columns(3).ColumnName = "Heater_Code"
            'ds.Tables("Heater_registration_Three1").Columns(4).ColumnName = "Heater_Tank"
            'ds.Tables("Heater_registration_Three1").Columns(5).ColumnName = "Shift_Line"
            'ds.Tables("Heater_registration_Three1").Columns(6).ColumnName = "Sap"
            'dg.DataSource = ds.Tables("Heater_registration_Three1")
            'Return True
    ...
    Private Function _Refresh155() As Boolean
        Try
            ...
            Dim ds As New DataSet
            da.Fill(ds, "Heater_registration_Three1")
            ds.Tables("Heater_registration_Three1").Columns(0).ColumnName = " Model Name"
            ds.Tables("Heater_registration_Three1").Columns(1).ColumnName = " Qty"
            dg9.DataSource = ds.Tables("Heater_registration_Three1")
            Return True
        Catch ex As Exception
        End Try
    End Function
